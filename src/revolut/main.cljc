(ns revolut.main
  "Kotodama WASM entrypoint for the Revolut clean-room actor (L4) — Clojure port.
  ...
  facts (`revolut.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "revolut")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"revolut.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "revolut-compat" :tier tier :entities entities} 200])
...
(defn start! [] :revolut-compat/ready)
